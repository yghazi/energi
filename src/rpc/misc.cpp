...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "energi|privatesend|instantsend|masternode|spork|keepass|mnpayments|mnsync|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "energi")
            + HelpExampleRpc("debug", "energi,net")
        );
...
#ifdef ENABLE_WALLET
        // Case 1: Energi address and we have full public key:
