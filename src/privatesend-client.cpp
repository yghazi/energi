...
void CPrivateSendClient::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Energi related functionality
...
bool CPrivateSendClient::MakeCollateralAmounts(const CompactTallyItem& tallyItem, bool fTryDenominated, CConnman& connman)
{
    ...
    bool fSuccess = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
            nFeeRet, nChangePosRet, strFail, &coinControl, true, ONLY_NONDENOMINATED_NOTCOLLATERALIFMN);
    if(!fSuccess) {
        LogPrintf("CPrivateSendClient::MakeCollateralAmounts -- ONLY_NONDENOMINATED_NOTCOLLATERALIFMN Error: %s\n", strFail);
        ...
        if(fTryDenominated) {
            ...
            if(!pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
                                nFeeRet, nChangePosRet, strFail, &coinControl, true, ONLY_NOTCOLLATERALIFMN)) {
                LogPrintf("CPrivateSendClient::MakeCollateralAmounts -- ONLY_NOTCOLLATERALIFMN Error: %s\n", strFail);
...
{
    if(fLiteMode) return; // disable all Energi specific functionality
